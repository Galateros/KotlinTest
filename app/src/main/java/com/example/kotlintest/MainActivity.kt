...
class MainActivity : AppCompatActivity() {
    var current : Int = 0
    val array = Array(7){100.0.pow(it)}
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        texto.text = database[current]
        Toast.makeText( this,  "Io Pienso Positivo", Toast.LENGTH_LONG).show()
    ...
    fun next(view : View){
        var pos = editText.text.toString().toInt()
        if(current < 1 || current >= database.size)
            texto.text = "Numero fuera de rango"
        else
            texto.text = database[pos]
    ...
    fun nextText(view: View){
        current = (current+1)%database.size
        texto.text = database[current]
    }
    fun previousText(view: View){
        current = (current+1)%database.size
        texto.text = database[current]
    }
